...
<section class="row bgFFF">
	...
	<article class="rc10">
		{% if aspect === '1' %}
		<p class="bg0d6077 colorFFF bold pad10x headRadius">Update Mode Activated</p>{% else %}
		<p class="bg0d6077 colorFFF bold pad10x headRadius">Entry Mode Activated</p>{% endif %}
{{ form_legacy(action) }}		
		<div class="display-table fsmall width100p ajaxForm dataBorder">
		    <div class="w22p">
		    	<div>Full Name</div><div>:</div><div><label>{{ text_field('fname','value':fname) }}</label></div>
		    </div>
		    <div>
		    	<div>Phone Number</div><div>:</div><div><label>{{ text_field('sphone','value':sphone) }}</label></div>
		    </div> 
		    <div>
		    	<div>Region</div><div>:</div><div>
					<select name="sregion" id="sregion">
					<option value="0">Reset &amp; Choose Category ...</option>
					{% for region in regions %}<option {% if region.rid === srid %}selected="selected"{% endif %} value="{{region.rid}}">{{region.regionname}}</option>{% endfor %}
					</select>
		    	</div>
		    </div> 
		    <div>
		    	<div>City</div><div>:</div><div>
					...
					<option value="0">Reset &amp; Choose Category ...</option>
					{% for city in citys %}<option {% if city.cid === scid %}selected="selected"{% endif %} value="{{city.cid}}">{{city.cityname}}</option>{% endfor %}
					</select>
		    	</div>
		    </div> 
		    <div>
		    	<div>Area</div><div>:</div><div>
					...
					<option value="0">Reset &amp; Choose Category ...</option>
					{% for area in areas %}<option {% if area.aid === said %}selected="selected"{% endif %} value="{{area.aid}}">{{area.areaname}}</option>{% endfor %}
					</select>
		    	</div>
		    </div>	        
		    <div>
		    	<div>Address</div><div>:</div><div><label>{{text_area('saddress','value':saddress)}}</label></div>
		    </div> 
			<div>
				<div>
				...
				{% if aspect === '1' %}				
					{{ submit_button('UPDATE & SHIP TO THIS ADDRESS','formaction':action,'enctype':'application/x-www-form-urlencoded','formmethod':'post','id':'shipupd','class':'marginAuto neumorphism cursorpoint width90p bold pad4x bgEEE') }}
				{% else %}
					{{ submit_button('SAVE & SHIP TO THIS ADDRESS','formaction':action,'enctype':'application/x-www-form-urlencoded','formmethod':'post','id':'shipupd','class':'marginAuto neumorphism cursorpoint width90p bold pad4x bgEEE') }}
				{% endif %}				
				</div>
			</div>
		</div>
{{end_form()}}		
		<div class="loader nodisp fxsmall txtCenter"><img alt="Loader" data-src="img/loading.gif" />Loading Expansion...</div>
